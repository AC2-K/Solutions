...
int main() {
    ...
    cin>>n;
    vector<int> a(n),h(n);
    for(auto&hh:h)cin>>hh,hh--;
    for(auto&aa:a)cin>>aa;
    //hは座圧不要

    SegmentTree<ll> dp(n,[](ll a,ll b){return max(a,b);},0);   //dp[i]:=末尾iとなるようなLISについて、\sum a_i の最大値
    ll ans=0;
    rep(i,n){
        ll val=dp.query(0,h[i]);
        if(dp.get(h[i])<val+a[i])dp.update(h[i],val+a[i]);
        chmax(ans,dp.get(h[i]));
    }
    cout<<ans<<endl;
}